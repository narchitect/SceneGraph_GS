...
class CocoMetric(BaseMetric):
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        #
        # # 디버깅 코드 추가: 예측 이미지 ID와 COCO 이미지 ID 비교
        # predictions = load(result_files['bbox'])  # 'bbox' 대신 적절한 키 사용
        # pred_img_ids = [pred['image_id'] for pred in predictions]
        # coco_img_ids = self._coco_api.getImgIds()
        #
        # if set(pred_img_ids) != set(coco_img_ids):
        #     logger.error(f"Predicted image IDs: {set(pred_img_ids)}")
        #     logger.error(f"COCO image IDs: {set(coco_img_ids)}")
